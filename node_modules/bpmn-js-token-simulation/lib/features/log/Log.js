...
export default function Log(
    ...
    scopeFilter, simulator, elementRegistry, graphicsFactory) {
  ...
  eventBus.on(TRACE_EVENT, event => {
    ...
    //change color if token passes element
    if (is(element, 'bpmn:SequenceFlow' || 'bpmn:MessageFlow')) {
      this.setColor(element, '#0e8d9e', '#c1dfec')
    }
    ...
    //change color if token passes element and add element to log-file
...
//set color function to change stroke & fill color of elements (called when element is passed by
//the token) also changes color of passed connection arrows between elements
Log.prototype.setColor = function(element, stroke, fill) {
