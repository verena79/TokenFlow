...
async function injectManifest(config) {
    const options = validate_options_1.validateInjectManifestOptions(config);
    // Make sure we leave swSrc and swDest out of the precache manifest.
    for (const file of [options.swSrc, options.swDest]) {
        options.globIgnores.push(rebase_path_1.rebasePath({
            file,
            baseDirectory: options.globDirectory,
        }));
    }
    const globalRegexp = new RegExp(escape_regexp_1.escapeRegExp(options.injectionPoint), 'g');
    const { count, size, manifestEntries, warnings } = await get_file_manifest_entries_1.getFileManifestEntries(options);
    let swFileContents;
    try {
    ...
    assert_1.default(injectionResults.length === 1, `${errors_1.errors['multiple-injection-points']} ${injectionPoint}`);
    const manifestString = fast_json_stable_stringify_1.default(manifestEntries);
    const filesToWrite = {};
    ...
    if (url && !url.startsWith('data:')) {
        const sourcemapSrcPath = upath_1.default.resolve(upath_1.default.dirname(options.swSrc), url);
        const sourcemapDestPath = upath_1.default.resolve(upath_1.default.dirname(options.swDest), url);
        let originalMap;
        try {
            // readJSON returns Promise<any>.
            // eslint-disable-next-line @typescript-eslint/no-unsafe-assignment
            originalMap = await fs_extra_1.default.readJSON(sourcemapSrcPath, { encoding: 'utf8' });
        }
        catch (error) {
            throw new Error(`${errors_1.errors['cant-find-sourcemap']} ${error instanceof Error && error.message ? error.message : ''}`);
        }
        const { map, source } = await replace_and_update_source_map_1.replaceAndUpdateSourceMap({
            originalMap,
