...
function getFileDetails({ globDirectory, globFollow, globIgnores, globPattern, globStrict, }) {
    ...
    try {
        ...
        throw new Error(errors_1.errors['unable-to-glob-files'] + ` '${err instanceof Error && err.message ? err.message : ''}'`);
    }
    if (globbedFiles.length === 0) {
        warning = errors_1.errors['useless-glob-pattern'] + ' ' +
    ...
    for (const file of globbedFiles) {
        ...
        const fileSize = get_file_size_1.getFileSize(fullPath);
        if (fileSize !== null) {
            const fileHash = get_file_hash_1.getFileHash(fullPath);
