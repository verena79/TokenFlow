...
async function getFileManifestEntries({ additionalManifestEntries, dontCacheBustURLsMatching, globDirectory, globFollow, globIgnores, globPatterns = [], globStrict, manifestTransforms, maximumFileSizeToCacheInBytes, modifyURLPrefix, templatedURLs, }) {
    ...
    try {
        for (const globPattern of globPatterns) {
            const { globbedFileDetails, warning } = get_file_details_1.getFileDetails({
                globDirectory,
                globFollow,
                globIgnores,
                globPattern,
    ...
    if (templatedURLs) {
        for (const url of Object.keys(templatedURLs)) {
            assert_1.default(!allFileDetails.has(url), errors_1.errors['templated-url-matches-glob']);
            ...
            if (Array.isArray(dependencies)) {
                ...
                allFileDetails.set(url, get_composite_details_1.getCompositeDetails(url, details));
            }
            else if (typeof dependencies === 'string') {
                allFileDetails.set(url, get_string_details_1.getStringDetails(url, dependencies));
            }
        }
    }
    const transformedManifest = await transform_manifest_1.transformManifest({
