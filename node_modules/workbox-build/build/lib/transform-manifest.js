...
async function transformManifest({ additionalManifestEntries, dontCacheBustURLsMatching, fileDetails, manifestTransforms, maximumFileSizeToCacheInBytes, modifyURLPrefix, transformParam, }) {
    ...
    if (maximumFileSizeToCacheInBytes) {
        transformsToApply.push(maximum_size_transform_1.maximumSizeTransform(maximumFileSizeToCacheInBytes));
    }
    if (modifyURLPrefix) {
        transformsToApply.push(modify_url_prefix_transform_1.modifyURLPrefixTransform(modifyURLPrefix));
    }
    if (dontCacheBustURLsMatching) {
        transformsToApply.push(no_revision_for_urls_matching_transform_1.noRevisionForURLsMatchingTransform(dontCacheBustURLsMatching));
    }
    ...
    if (additionalManifestEntries) {
        transformsToApply.push(additional_manifest_entries_transform_1.additionalManifestEntriesTransform(additionalManifestEntries));
    }
