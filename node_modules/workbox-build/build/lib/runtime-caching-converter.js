        ...
        switch (optionName) {
            ...
            case 'backgroundSync': {
                ...
                if (options.backgroundSync.options) {
                    pluginCode += `, ${stringify_without_comments_1.stringifyWithoutComments(options.backgroundSync.options)}`;
                }
                pluginCode += `)`;
            ...
            case 'broadcastUpdate': {
                ...
                plugins.push(`new ${plugin}(${stringify_without_comments_1.stringifyWithoutComments(opts)})`);
                break;
            }
            case 'cacheableResponse': {
                const plugin = moduleRegistry.use('workbox-cacheable-response', 'CacheableResponsePlugin');
                plugins.push(`new ${plugin}(${stringify_without_comments_1.stringifyWithoutComments(options.cacheableResponse)})`);
                break;
            }
            case 'expiration': {
                const plugin = moduleRegistry.use('workbox-expiration', 'ExpirationPlugin');
                plugins.push(`new ${plugin}(${stringify_without_comments_1.stringifyWithoutComments(options.expiration)})`);
                break;
            }
            case 'precacheFallback': {
                const plugin = moduleRegistry.use('workbox-precaching', 'PrecacheFallbackPlugin');
                plugins.push(`new ${plugin}(${stringify_without_comments_1.stringifyWithoutComments(options.precacheFallback)})`);
                break;
            }
            case 'rangeRequests': {
            ...
            default: {
                ...
                throw new Error(`${errors_1.errors['bad-runtime-caching-config']}  ${optionName}`);
            }
        }
    }
